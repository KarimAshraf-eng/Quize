...
class QuizApp {
    ...
    async loadLectures() {
        ...
        try {
            ...
            // --- الكود الجديد: قائمة بأسماء ملفاتك ---
            // أضف أسماء ملفات JSON الخاصة بك هنا بالترتيب الذي تريده
            const customFileNames = [
                ...
                'Lecture_5.json',
                'MidtermExam.json'
                // 'MyMidtermExam.json', // <-- مثال لملف جديد
                // 'Final_Review.json'   // <-- مثال لملف آخر
            ];

            // --- الكود الجديد: حلقة تمر على القائمة ---
            for (let i = 0; i < customFileNames.length; i++) {
                const fileName = customFileNames[i];
                const lectureNumber = i + 1; // الرقم الترتيبي للمحاضرة
                // نرسل الرقم واسم الملف إلى الدالة
                lecturePromises.push(this.loadLectureFile(lectureNumber, fileName));
            }
            // ----------------------------------------------------
            ...
            results.forEach((result, index) => {
                if (result.status === 'fulfilled' && result.value) {
                    // نستخدم الرقم الترتيبي كمفتاح
                    this.lectures.set(result.value.number, result.value);
                }
            });
    ...
    // ------------------- (التعديل الثاني هنا) -------------------
    async loadLectureFile(lectureNumber, fileName) { // <-- أضفنا "fileName"
        try {
            // --- الكود الجديد: نستخدم اسم الملف "fileName" ---
            ...
            return {
                number: lectureNumber, // <-- نستخدم الرقم الترتيبي
    ...
    updateQuizHeader() {
        ...
        // --- الكود الجديد: يتعامل مع العناوين المخصصة ---
        const lectureText = this.sessionData.currentLanguage === 'en' ?
            this.currentLecture.title_en : this.currentLecture.title_ar;
        // ---------------------------------------------
    ...
    renderQuestion() {
        ...
        // **FIX: Re-show feedback if already answered (and not in 'retry')**
        const hasAnswer = this.sessionData.answers.has(questionKey);

        if ((this.viewOnlyMode || this.answerSubmitted) && hasAnswer) {
            // "answerSubmitted" is true if we just submitted OR just pressed "back"
    ...
    renderChoices(question) {
        ...
        choices.forEach(choice => {
            ...
            } else if (this.answerSubmitted && userAnswer) {
                // RETRY mode AFTER submitting, OR returning with "Back" button
                const correctChoice = question.correct_choice;
                // **FIX: Use the stored answer, not currentSelection (which is null on 'back')**
    ...
    updateQuizControls() {
        ...
        // **FIX: OK button is disabled if in viewOnlyMode, or if answerSubmitted is true**
        ...
        // **FIX: Show Next button if in viewOnlyMode, or if answerSubmitted is true**
    ...
    previousQuestion() {
        if (this.currentQuestionIndex > 0) {
            ...
            // **FIX: Set answerSubmitted to true IF an answer exists OR we are in viewOnlyMode**
            this.answerSubmitted = this.sessionData.answers.has(questionKey) || this.viewOnlyMode;
    ...
    toggleExplanationLanguage() {
        ...
        if (!feedback.classList.contains('hidden')) {
            ...
            const questionKey = `${lectureNum}-${question.question_number}`;
            const userAnswer = this.sessionData.answers.get(questionKey);
            // Use currentSelection if it exists, otherwise fall back to saved answer
            const userChoice = this.currentSelection || (userAnswer ? userAnswer.choice : null);
    ...
    // Dashboard
    showDashboard() {
        // **FIX: Ensure we are not in a special mode when showing dashboard**
        this.exitReviewMode();
        this.exitViewOnlyMode();
    ...
    calculateResults() {
        ...
        // Calculate favorites count for this lecture
    ...
    renderResultsTable() {
        ...
        this.currentResults.wrongAnswers.forEach(({ question, userChoice, questionKey }) => {
            ...
            row.innerHTML = `
                ...
                <td>${this.currentLecture.title_ar}</td>
                <td>${userChoice}</td>
